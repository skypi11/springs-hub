...
export default function AdminLinkChangesPage() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const data = await api<{ requests?: ChangeRequest[] }>('/api/admin/rl-link-changes');
  ...
  async function decide(r: ChangeRequest, decision: 'approve' | 'reject') {
    ...
    setActingId(r.id);
    try {
      await api(`/api/admin/rl-link-changes/${r.id}`, {
        method: 'PATCH',
        body: { decision },
      });
  ...
  return (
    <>
      <div className="flex items-center gap-3">
        <RefreshCw size={18} style={{ color: 'var(--s-gold)' }} />
        <h2 className="font-display text-xl" style={{ letterSpacing: '0.04em' }}>
          CHANGEMENTS DE COMPTE EPIC ({pendingCount})
        </h2>
      </div>
