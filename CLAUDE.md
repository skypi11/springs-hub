...
- **Phase 3** — Compétitions natives Aedral : **BUILD LANCÉ le 02/07** pour la **Legends Springs Cup** (circuit RL : 4 Qualifs 26-27/09, 10-11/10, 24-25/10, 7-8/11 + LAN 21-22/11 ; inscriptions Qualif 1 le 12/09 = deadline dure). **Sources de vérité : [docs/legends-springs-cup-spec.md](docs/legends-springs-cup-spec.md) (spec fonctionnelle complète, validée par Matt) + [docs/legends-cup-architecture.md](docs/legends-cup-architecture.md) (plan d'archi v2 post-review adversariale, phasage Lots 0→5 + jalons)** — TOUJOURS lire ces deux docs avant de toucher au module compétitions. Points structurants : moteur générique (Legends Cup = une instance), birthDate → `user_secrets`, admins compétition scopés (`competition_admins`), règlement versionné avec acceptation à l'inscription (organisateur légal : SPRINGS E-SPORT, orthographe exacte). **Stratégie de dev — CHANGÉE le 17/07 : `legends/lot-2` A ÉTÉ FUSIONNÉE SUR MAIN, on travaille désormais DIRECTEMENT sur main.** L'ancienne stratégie (branches courtes `legends/*` + previews) a été abandonnée sur décision de Matt : les deux branches divergeaient à chaque chantier hors-compét, chaque report en prod passait par un cherry-pick manuel, et le risque d'erreur de résolution dépassait celui de livrer du code gaté (le 16/07, le 1er vrai conflit est apparu sur `my-structure/page.tsx`). **Ce qui protège la prod est le FEATURE GATING, pas la branche** : `isCompetitionHidden` = `status==='draft' || isDev===true` → 404 public, appliqué sur les 11 routes + doublé dans les rules. Vérifié à la fusion : les 3 compétitions et le seul circuit en base sont tous masqués, et un anonyme sur `aedral.com/competitions/demo-bracket-view` reçoit « PAGE INTROUVABLE » (nom de la démo : 0 occurrence dans le HTML ; `/api/competitions/circuits` → `{"circuits":[]}`). **Avant de publier une compétition, se rappeler qu'elle devient publique dès `status!=='draft' && !isDev`.** La DB Firebase reste partagée local/prod → données de test en compétitions `draft`/`isDev`, prudence absolue sur firestore.rules (global).
  ...
  - **Lot 3 — Jour de match : QUASI COMPLET (lancé et bâti les 11-12/07, sessions Fable 5, sur la même branche `legends/lot-2`).** Serveur : machine d'états match PURE (`lib/competitions/match-flow.ts`) + progression = UNIQUE chemin d'écriture d'un résultat (`lib/competitions/progression.ts`, diff moteur → patchs ciblés, jamais d'écrasement des champs jour-de-match) + routes (page de match participant avec camp dérivé serveur `match-access.ts`, tick idempotent avec réparation, console admin : launch_phase partiel + rooms + forfaits + force-score + cast + reopen_checkin + check-in général + withdraw R5-4 + replace waitlist). **Review adversariale CRITIQUE 61 agents** (2 blockers règle-de-course corrigés par la garde `autoGuard` re-validant l'état frais dans la transaction ; rules Firestore défense-en-profondeur compéts masquées DÉPLOYÉES + champ `hidden` sur les matchs). **e2e `scripts/e2e-legends-matchday.mjs` 31/31** (tournoi complet via les vraies routes, y compris cascade DIFFÉRÉE de DQ). UI : **page de match** `/competitions/[id]/match/[matchId]` (héros, check-in + countdown, room copiable, saisie des manches, litige — bracket cliquable) + **console live** `/admin/competitions/[id]/console` (phases, rangées denses, modales, équipes/waitlist). Doc présidence : `public/legends/fonctionnement.html` (comportements situation par situation pour Teitei/Riskone — À TENIR À JOUR). ✅ **3D + 3G livrés (aaf07c3, 12/07)** : captures de litige (upload capitaine/staff pendant litige, cap 10/camp transactionnel, servies en URLs SIGNÉES 300 s aux seuls participants+admins — keys R2 privées) + `lib/competitions/withdraw-registration.ts` PARTAGÉ (retrait par l'équipe depuis l'onglet Inscriptions — dirigeant/responsable du jeu/manager d'équipe, refus explicite si bracket publié) + hook `applySanctionEffect` (exclusion/ban → retrait auto des inscriptions actives du périmètre ; équipe en bracket publié = jamais de DQ auto, notif admins). **Preuves : e2e matchday 38/0 CONTRE preview.aedral.com (jambe R2 réelle — le secret R2 est « Sensitive » Vercel, jamais dispo en local ; l'e2e supporte `E2E_BASE_URL` + `VERCEL_AUTOMATION_BYPASS_SECRET`) + e2e withdraw 7/0 + 848 tests.** ✅ **MOTEUR SIMPLE ÉLIMINATION SHIPPÉ (4f527bf, 12/07)** : multi-format de la config au bracket public — `generateSingleElim` + petite finale optionnelle (`thirdPlace`, match P3 mappé « consolation final » NATIVE du viewer), placements dédiés (isFinished exige la petite finale réglée), pont/validation/form kind-aware (préréglage « Tournoi en ligne », cross-check kind↔plan de phases), libellés par kind. Review adversariale CRITIQUE 31 agents → 3 majors corrigés (dont un bug de placement prouvé par exécution) + 3 durcissements, reports documentés en mémoire. Preuves : 915 tests, e2e `scripts/e2e-legends-single-elim.mjs` 20/0 (tournoi complet via les vraies routes), Playwright OK. Démo `demo-single-elim` (isDev) laissée en base (DB partagée → visible sur preview), rejouable via `scripts/seed-demo-single-elim.mjs`. Le tournoi test de Matt est DÉBLOQUÉ. ✅ **Timers Web Worker shippés (d336c5f)** — hook `useWorkerInterval`, polling/tick survivent aux onglets en arrière-plan (console + page match + BracketView). ✅ **LOT 4 FONCTIONNELLEMENT COMPLET (12/07)** : **4A+4B clôture d'un Qualif (f9b2b2d)** — `lib/competitions/close-competition.ts` (transaction unique : placements compressés + arbitrages, points du barème, participations circuit_teams, purge claims waitlisted, live→finished ; équipe DQ garde place+points R5-4), `computePlacements(bracket, tiebreakResolutions)`, console actions resolve_tiebreak/close_competition + TiebreakCard drag&drop + CTA « Clôturer le Qualif » + classement final ; e2e single-elim 27/0. **4C fil de match (0b12ff6)** — route thread (staff résolu LIVE, réponse sans uid, limiter chat 20/30s), rules /messages deny-all DÉPLOYÉES (sync vieux site 2606ee9), UI MatchThread ; e2e matchday 40/0. **Review adversariale critique du Lot 4 TRIÉE ET CORRIGÉE (5512194)** : 2 blockers (repêchage atomique dans la tx moteur ; unapprove refusé après bracket publié + clôture dérivée du BRACKET avec `unlinked` remonté) + majors (garde live dans la tx moteur, resolve_tiebreak gate vraie égalité, fil limiter-uid/auth-first/dualStaff, UI scroll/IME/worker/anti-double-submit) — e2e 30/0 + 40/0, 919 tests ; reports assumés en mémoire (goalDiff float, barème court, cap messages, modales snapshot). **Restent : petits retours UI/UX Matt (Opus 4.8), repêchage LAN (hors scope, au moment de la LAN), Lot 5 (tournoi réel + runbook).** — le classement final EST affiché sur la fiche publique (`FinalStandings` dans `app/competitions/[id]/page.tsx`, place/points/diff par match/podium) : cette ligne « restante » traînait périmée et m'a fait annoncer à tort la feature comme manquante dans la notice présidence du 28/07. Le vrai tournoi en ligne de Matt (avant la Legends) est débloqué : simple élim + clôture + points bout en bout. skypi11 est capitaine de Nova Legion sur la démo (L1-2 live) pour tester le parcours capitaine sur preview. Voir mémoire `project_legends_lot3`. **✅ USINE À TOURNOIS — MOTEURS ROUND ROBIN + SUISSE + REGISTRY DE FORMATS SHIPPÉS (23-24/07, session Fable 5).** Suisse (`85ad5cc`+`a91f102`) : GÉNÉRATION INCRÉMENTALE (ronde N+1 appariée sur les résultats — action console `generate_next_round`, publish = ronde 1 seule), Monrad avec backtracking COMPLET (bye inclus), Buchholz, **SOUPAPE `isSwissStuck`** (un suisse structurellement inappariable — retraits/K3,3 — devient clôturable au classement courant, jamais de compétition `live` briquée). ⚠️ La review suisse a été COUPÉE par la limite de dépense mensuelle (21 réfuteurs morts en vol) → findings re-triés à la main + **3 fichiers `__probe*.test.ts` orphelins purgés (2 avaient été poussés par `git add -A` — TOUJOURS `git status` avant add -A post-review)**. **✅ VUE DES POULES/RONDES SHIPPÉE (24/07, `9043b41`)** : stage round_robin natif du viewer + API `/standings` (classement officiel par les fonctions pures, cache CDN) + StandingsTable DA + CTA console « Apparier la ronde suivante » + `scripts/seed-demo-groups.mjs` (démos isDev `demo-round-robin`/`demo-swiss` EN BASE, flux prouvé e2e via vraies routes dont `generate_next_round` réel). **Rendu VALIDÉ par Matt (25/07)** après 3 itérations de retours (poules fusionnées → fix `group` console + CSS colonnes ; table compactée ; « Égalité » gatée sur `concluded`) + 2 fixes admin au passage (hard refresh n'éjecte plus l'admin — course `profileEnriched`, `app/admin/layout.tsx` ; `/admin/competitions/[id]` → redirect console). **DÉCISION MATT (25/07) : FINIR L'USINE À TOURNOIS AVANT TOUT AUTRE CHANTIER — ① runtime multi-étapes (Fable) ✅ FAIT, ② seeding MMR/circuit (Fable) ✅ FAIT, ③ refonte création + réglages Discord (Opus 5) ✅ FAIT le 25/07 — L'USINE À TOURNOIS EST CLOSE.** ③ en détail (mémoire `project_creation_form_discord`) : **cap 64 équipes** (`5213fea`, 128 refusé sciemment — 256 matchs > budget atomique d'étape de 200) ; **page de création refaite** (`91b3b79`+`7afcdbf`) — écran de choix à 6 préréglages puis un seul écran, champs pilotés par la registry (l'écran ne connaît AUCUN format), multi-étapes créable à la souris (avant : script obligatoire), **déroulé en langage d'organisateur** (« Huitièmes — 8 matchs » au lieu de « WR2 + LR1 », ordonnancement double élim par dépendances prouvé 4→64), validation en direct avec le message exact du serveur ; **retours Matt** (`1ce5518`) — bug de suppression de jour (deux `setState` depuis le même instantané), régression de l'éditeur de BO remis **par nom de tour** (test miroir contre le moteur), déroulé ouvert à TOUTES les étapes (`advance_stage` respecte le plan prévu), repères visuels par jour ; **réglages Discord** (`43649a0`+`e995c55`) — garde « admin du serveur » (trou préexistant : on pouvait provisionner le Discord d'une autre structure), rôles staff qui accèdent aux salons, salon d'annonces + salon d'alertes staff, message d'accueil, nettoyage manuel, débordement au-delà de 50 salons/catégorie ; **vitrine des tournois hors circuit** (`03671d1`, ils n'étaient listés nulle part). **PIÈGES DISCORD À NE PAS REFAIRE** : `/guilds/{id}/members/@me` n'existe pas pour un bot (400) — passer par `/users/@me` puis l'id ; **une mention dans un embed ne ping PERSONNE** (seul le `content` notifie — les check-ins ne réveillaient aucun joueur) ; 50 salons max par catégorie ; le rôle du bot doit être haut dans la hiérarchie. **REVIEW ADVERSARIALE 66 agents** (4 lentilles × 2 réfuteurs) : 31 findings bruts, 28 survivants, **TOUS corrigés** (`b32197b` les 6 blockers + `c91fce3` les 22 restants), avec tests de régression. Les 3 blockers les plus instructifs : (1) le plan de phases passé au moteur DOIT être filtré par étape (`phasePlanForStage`) — le rattachement se fait par (bracket, round) sans notion d'étape, donc le plan complet écrasait les phases de l'étape 1 par celles de l'étape 2 ; (2) une garde d'accès à un serveur ne vaut RIEN si les cibles (salons, rôles) ne lui sont pas rattachées — les écritures Discord ignorent le serveur ; (3) ne jamais supprimer sur un critère déduit (registre explicite `discord.createdChannelIds`/`categoryIds` de ce que le bot a créé). Plus : cascade des effectifs sur TOUTE la séquence d'étapes, saisie numérique recadrée au blur et non à la frappe, BO en liste de valeurs (type de champ `choice` dans la registry). **RESTE (demandé par Matt, dans l'ordre)** : vérification du bot côté STRUCTURE (contrôle après installation + état permanent + notification au fondateur quand ça casse — le bot est sur 13 serveurs, aucun n'est contrôlé), bouton « inviter le bot sur un autre serveur » pour tous les fondateurs, page publique « ce que fait le bot » + pages légales à jour. **SESSION 27/07 — LE PROVISIONNING DISCORD ENFIN PROUVÉ + « LE BOT RACONTE LE TOURNOI » LANCÉ.** Déclencheur : Matt signale que l'usine à tournois n'est pas finie — « pas mal de tests non réalisés (le bot Discord qui crée les salons) », plus deux trous : le bot muet aux moments clés, et l'UI/UX du module compétition jamais travaillée au-delà des écrans #1→#5. **(A) Provisioning Discord prouvé** (`8ac3224`) : tout `lib/discord-competition.ts` (20 appels Discord) n'avait AUCUN test — code reviewé, jamais exécuté. Nouveau **`scripts/e2e-competition-discord.mjs` (69 vérifications)** contre un VRAI serveur (celui de Matt, guild `1531385106139189269`, bot admin) : déroule console → route → Discord puis **RELIT l'état chez Discord** (un 200 ne prouve rien des permissions posées), nettoie en `finally`. Couvre rôle participant, catégorie, salons annonces/staff/équipe + overwriteset, message d'accueil **et son ping**, attribution des rôles, joueur absent, idempotence, déprovision, nettoyage épargnant le salon de l'organisateur, **circuit à 2 étapes partageant le rôle participant**, et **débordement > 50 salons/catégorie** (option `--overflow` : le cas des 32 équipes × 2 salons). **2 bugs trouvés par l'exécution** : (1) refuser/retirer une équipe retirait le rôle participant à ses joueurs sans vérifier s'ils jouaient ailleurs — or ce rôle est COMMUN AU CIRCUIT, donc un joueur validé Qualif 1 puis refusé Qualif 2 perdait l'accès aux salons ; corrigé au centre (`discordIdsStillParticipating`), une lecture ratée conserve le rôle plutôt que de couper à tort ; (2) `describeGuildAccess` avertissait « remonte le rôle du bot » dès la position 1 = **tout serveur fraîchement invité** — faux positif prouvé faux par exécution (rôle créé en position 1, attribution 204). **(B) Publier un circuit — BLOQUANT 12/09 levé** (`4534358`) : le PATCH ignorait `status` (« évolue avec les lots suivants ») et `isCircuitHidden = status==='draft'` → **aucun chemin pour rendre un circuit public**, la Legends était condamnée au 404. Action publish/unpublish idempotente, retour en brouillon refusé dès qu'une étape est terminée, verrou d'édition assoupli (nom/dotation/organisateur/places LAN corrigeables ; jeu/barème/bestResultsCount/départages figés — comparaison insensible à l'ordre des clés, sinon changement fantôme), bouton + statut en or dans `/admin/competitions`. e2e `scripts/e2e-circuit-publish.mjs` 17/17 (visibilité prouvée par lecture ANONYME). **(C) « Le bot raconte le tournoi »** (`e51965c`) : cartographie multi-agents (8 zones + 2 critiques) → **123 moments, 72 sans aucun message**. **Source de vérité : [docs/bot-tournoi-plan.md](docs/bot-tournoi-plan.md)** (décisions + lots + **ce qu'on n'enverra JAMAIS**). Socle : `lib/competitions/broadcast-messages.ts` (textes purs testés, voix du skill aedral-style) + `lib/competitions/tournament-broadcast.ts` (**seul chemin d'envoi** : résolution des salons, règle de mention unique — annonces JAMAIS pingées —, garde isDev, deadline, **accusé de livraison**). Livré : room jointe au check-in (le bot la promettait sans jamais la donner), check-in par camp + lien vers LE MATCH, check-in relancé, forfait/score forcé avec motif aux 2 camps, retrait + cascade aux adversaires, bracket publié → adversaire de chaque équipe. **Et une SUPPRESSION** : `single_entry` ne ping plus le staff (déroulement normal → des dizaines de pings « tout va bien » sur 63 matchs → salon coupé, vrais litiges perdus). **LOTS 1 ET 2 CLOS ET PROUVÉS PAR EXÉCUTION (28/07).** `scripts/e2e-tournament-broadcast.mjs` (67 vérifications) déroule un tournoi entier contre le VRAI serveur Discord et **relit chaque message posté** (contenu, mention, lien, salon) : bracket publié → adversaire par équipe + annonce publique NON pingée, check-in avec room+mot de passe et lien vers LE match, contre-saisie signalée au camp qui n'a pas saisi, litige aux 2 équipes, arbitrage aux 2 camps avec motif, relance T-5 des seuls retardataires (verrou transactionnel anti-double-envoi), garde isDev prouvée (une démo n'écrit RIEN). Piège du test lui-même : il supposait que le seed 1 affronte le seed 2 — en seeding standard c'est le DERNIER seed ; l'adversaire est désormais dérivé du bracket. **Lot 2 livré** : action **Annonce** texte libre (3 cibles, accusé de livraison dans le toast), **« Vérifier le dispositif »** (`lib/competitions/setup-check.ts` — bot/droits/salons/rôle participant/équipes provisionnées/**joueurs absents du serveur**/inscriptions sans décision ; `partial` compte comme provisionné, sinon un joueur absent faisait crier au blocage), relance T-5. **Intent « Server Members » ACTIVÉ par Matt le 28/07** → le contrôle lit la liste en 1 appel (fallback unitaire borné conservé si 403) ; pagination `after` plafonnée à 10 000 (sans elle, tout membre au-delà du millième était annoncé « absent » — 5 tests contre un Discord simulé). Aussi : **bouton « Inviter sur un autre serveur »** (URL d'invitation simple, sans liaison — le besoin réel de Matt : un Discord pour les équipes, un autre pour les compétitions qu'il organise ; l'archi le couvrait déjà, `competition.discord.guildId` est indépendant de la structure) + **liste déroulante des serveurs** dans le formulaire de compétition (`/api/admin/competitions/discord-guilds`, ceux où le bot est présent ET qu'on administre) avec notice et repli en saisie manuelle. **Filet cron ABANDONNÉ à raison** (Matt : « ça a l'air d'être une galère ») : la page de match des joueurs envoie déjà le tick, l'organisateur est présent au check-in général, et le bouton « Forcer les échéances » couvre le rattrapage — seul le refactor `lib/competitions/tick-runner.ts` a été gardé. **RESTE : Lot 3** (fin de parcours qualifié/éliminé, podium, réserve prévenue le jour J, rappel J-1, relais du fil de match dans les 2 sens — tous vérifiés absents du code le 28/07), **puis la PASSE UI/UX du module compétition** (jamais faite sur page de match, console, classements, multi-étapes, seeding, roster, création ; écrans #6 inscriptions-tab et #7 règlement jamais faits). **DROITS DES ADMINS DE COMPÉTITION OUVERTS (30/07, `dee456c`+`61a1ddb`)** — signalé par Matt : « c'est normal qu'ils ne peuvent pas créer une compétition ? ». C'était un choix du Lot 0 (mutations = admins Aedral) devenu faux : chaque Qualif passait par Matt, et le rôle promettait l'inverse de ce qu'il permettait. Ouvert : POST/PATCH/DELETE `competitions` (draft-only conservé) **+ les 2 routes dont la création dépend** (`discord-guilds` la liste, `discord-guild` l'inspection salons/rôles — sans elles ils créaient un tournoi sans pouvoir le configurer ; trou que Matt a aussi repéré : « pourquoi un admin compét n'a pas de liste Discord ? »). **Restent fermés à raison** : les CIRCUITS (barème/bestResultsCount/lanTeamCount = le contrat sur 3 mois), la nomination d'admins (escalade), le logo d'organisateur (n'existe que dans CircuitForm), le bac à sable, la vue R2. e2e `scripts/e2e-competition-admin-rights.mjs` 15/15 avec un compte dans `competition_admins` et PAS dans `aedral_admins` (**le piège : `isCompetitionAdmin` est vrai pour les deux rôles**, un test avec le compte de Matt n'aurait rien prouvé) : mutation réelle prouvée (suppression d'un brouillon) + refus vérifiés + règles du circuit intactes après coup. **DÉMO DISCORD REJOUABLE** : `scripts/demo-discord-competition.mjs` (8 équipes crédibles, 4 déroulements distincts — accord silencieux / litige+arbitrage / forfait / retrait+cascade —, rôle « Staff démo » créé pour que des invités voient les salons privés, compétition laissée VISIBLE pour que les liens résolvent, room vérifiée non-fuitante en anonyme ; sorties `--hide` et `--cleanup`). Notice présidence à jour et PUBLIQUE : `https://aedral.com/legends/fonctionnement.html` (un lien `*.vercel.app` passe, lui, par le mur SSO). Voir mémoire `project_bot_raconte_tournoi`. **SESSION 02/08 — L'AFFICHE DE RÉSULTAT REFONDUE ET VALIDÉE PAR MATT.** Déclencheur : « ça manque d'âme… les structures mettent des affiches qui claquent, sinon les gens scrollent sans regarder ». L'affiche était une fiche de score (fond noir, tout centré, deux camps au même poids). Refonte en 6 allers-retours avec Matt (`6c81732`→`f75dcc7`) : **elle porte désormais l'ORGANISATEUR, pas Aedral** (couleur `accentColor` + logo + nom, repli compétition→circuit→couleur du jeu ; « aedral.com » réduit à une mention grise), **visuel du jeu en fond**, diagonale du côté du vainqueur, déséquilibre gagnant/perdant assumé, et **carré recomposé EN PILE** (c'était un 16:9 étiré avec un tiers de vide). **PIÈGES DE CALIBRAGE (chacun a coûté une itération, tous dans la mémoire `project_affiche_resultat`)** : l'assombrissement de l'image et la vignette CSS **se MULTIPLIENT** (à 0.78 il restait 7 % du visuel, à 0.55 environ 25 % → on ne voyait pas le jeu ; valeur retenue 0.43) ; **la saturation décide plus que la luminosité** si un fond reste un fond (0.34) ; **le cadrage doit être calculé AU RATIO DE SORTIE** — une bande fixe forçait un agrandissement ×1.67 sur le carré, d'où la pixelisation que Matt a vue ; **éclaircir révèle le bruit de compression** des zones sombres ; garde-fou `BACKGROUND_MAX_UPSCALE`. Outil `scripts/render-poster.mjs` (rendre les 2 formats en local : ni tsc ni les tests ne disent rien d'une image satori). **3 BUGS TROUVÉS EN CHEMIN** : (1) `organizer`/`accentColor` étaient validés à l'écriture mais ABSENTS de `serializeCompetition` → le formulaire d'édition les EFFAÇAIT à chaque enregistrement ; (2) `tm.webp` fait 768×219 → aucune affiche Trackmania n'aurait eu de fond ; (3) `valorant-banner.jpg` est le LOGO sur aplat, pas une scène → fond rouge. Nouveaux visuels dans `public/og-backgrounds/` (1920×1080, celui de RL FOURNI PAR MATT) — dossier SÉPARÉ des visuels du site qui servent `games-registry` et la page profil. Aussi : champs organisateur/couleur ajoutés au formulaire de compétition avec le **logo en UPLOAD** (`OrganizerLogoField` partagé avec `CircuitForm` — Matt : « avec un lien celui-ci peut casser et les gens vont penser que c'est le site qui ne fonctionne pas »), et **un seul format dans le salon Discord** (le 16:9 en image, le carré en lien sur la même ligne via `extraLinks`/`buildEmbedDescription`, 6 tests) — le commentaire du code promettait ce lien depuis le 30/07 sans qu'il existe. **RESTE INCHANGÉ : Lot 3 du bot, puis la passe UI/UX du module compétition** (y noter le bouton « Télécharger l'affiche » 2 formats sur la page de match, et l'affiche de RENCONTRE hors compétition restée sur l'ancienne DA — tout est dans `docs/backlog-competitions.md`). Voir mémoire `project_affiche_resultat`.

**POUR LA LEGENDS : le bot n'est sur AUCUN serveur nommé « Springs E-Sport »** — à inviter avant le 12/09 (`https://discord.com/oauth2/authorize?client_id=1495127667203113072&permissions=8&scope=bot`), sinon aucun provisioning possible. **✅ SEEDING MMR/CIRCUIT + PANNEAU CONSOLE SHIPPÉS (24/07, commits `236c9df` build + `6bd0740` correctifs review)** : design [docs/registry-formats-design.md](docs/registry-formats-design.md) **§10**. Valeur de seed = `computed.worstLineupAvg` (compo la plus forte, anti-smurf) ; ordre circuit par `computeCircuitStandings` (non-classées après, par MMR) ; action bracket `seed_by {random|mmr|circuit}` ; transferts multi-étapes reseed `mmr`/`circuit` (la QUALIFICATION reste au classement, seul l'ordre de seed change). **SeedingPanel (console) = PREMIÈRE UI de pilotage du seeding** — avant, publier un bracket exigeait un script ; Matt peut désormais dérouler un tournoi entier seul (ouvrir seeding → stratégie 1-clic → ↑/↓ → publier). Review 31 agents → **publish rendu TRANSACTIONNEL** (verrou `bracketMaterializedAt` posé en tête + regs relues fraîches par doc-id + `approvedCount` ; approve gaté post-matérialisation — waitlist reste ouverte pour le repêchage ; statut live au dernier batch, crash mi-publication = re-publier répare) + soupape stuck neutralise la stratégie au dry-run + seed_by/shuffle/reorder en TX courtes + `seedingStale`/`canPublish` honnêtes au GET. Preuves : 1266 tests, e2e `scripts/e2e-legends-seeding.mjs` 9/9 + multistage 34/34 rejoués, flux console vérifié au navigateur. **Piège levé : l'auth admin Playwright headless FONCTIONNE depuis le fix `profileEnriched` du 25/07 (cookie aedral_auth + /?auth=1) — le contrôle visuel de la console n'est plus « prod uniquement ».** **✅ CHANGEMENT DE ROSTER POST-VALIDATION SHIPPÉ (25/07, commits `42fa5ed` build + `5cc8d13` garde du noyau + `06e5d1c` correctifs review, session Fable 5)** : dérogation ADMIN au roster lock (spec §4 — le lock reste TOTAL côté structure, philosophie dérogations d'âge), cas cible = titulaire malade la veille/en cours du 12/09. 3 opérations (`change_roster` : replace [rôle HÉRITÉ du sortant — effectifs invariants], swap_roles, set_captain) via `lib/competitions/roster-change.ts` (pur) + route registrations + `RosterEditor` inline ZONE B du Dossier. Les checks du WIZARD sont rejoués (l'approve ne les rejoue pas) : équipe Aedral, anti-doublon (+ équipes RETIRÉES ayant joué), sanctions, vérifié, mineur→note dérogation 422, MMR admin→refMmr ; computed recalculé ; **GARDE DU NOYAU** (demande Matt anti-« petits malins ») : titulaires validés figés au 1er changement (`initialStarterUids`), roster courant doit en garder ⌈2n/3⌉ sinon refus net (retrait+repêchage = le chemin) ; **historique visible** dans le Dossier (rosterChanges avec noms). Accès matchs auto (match-access relit LIVE) ; ACL non-terminaux alignées ; remplacer LE capitaine transfère le pilotage à l'entrant (notifié). Review critique 50 agents → 1 blocker corrigé (TX : TOCTOU statut par ÉGALITÉ + relecture de TOUS les matchs + garde de révision `matchesRevision` incrémenté par publish/generate_next_round/advance_stage) + approve gardé sur rosterUids + sélecteur avec agrégat smurf + UX 409/422. Preuves : 1279 tests, e2e `scripts/e2e-legends-roster-change.mjs` 25/25. Report : promotion waitlist sans identité circuit (trou préexistant replace_team). Le self-service structure (demande capitaine → validation) = UI Opus sur cette mécanique. **✅ RUNTIME MULTI-ÉTAPES SHIPPÉ (24/07, commits `7a2ec57` build + `b5a5c28` correctifs review, session Fable 5)** : un tournoi = une SÉQUENCE d'étapes de format (poules RR/suisse → top-N re-seedé au classement → phase finale élim), design contractuel [docs/registry-formats-design.md](docs/registry-formats-design.md) **§9** (à LIRE avant de toucher au multi-étapes). Livré : `Competition.stages/currentStage/stageResults` (rétrocompat totale : sans `stages` = mono-étape strictement identique ; INVARIANT `stages[0].format === comp.format`), ids d'étape ≥ 2 préfixés `E{n}_` par le PONT (`lib/competitions/stage-ids.ts` client-safe + `stages.ts` runtime pur — moteurs intouchés), reconstruction PAR ÉTAPE (`reconstructBracket({stage})`), action console **`advance_stage`** (toutes égalités arbitrées exigées, retirées jamais qualifiées = repêchage au classement, TX avec re-checks currentStage/withdrawn/résolutions, borne 200 matchs/passage), clôture = étape finale ++ éliminées des étapes closes recompressées + stats cumulées, **SOUPAPE `isStageTransferStuck`** (transfert structurellement impossible → clôturable au classement courant, miroir isSwissStuck), `/standings` sert l'ordre OFFICIEL par priorité (stageResults → finalPlacements → résolutions), TournamentBracket = sélecteur d'étape interne (une seule instance viewer — singleton), fiche à onglets Poules/Phase finale. Review adversariale critique 39 agents → 4 majors corrigés (garde currentStage dans la TX resolve_tiebreak ; standings finale jamais figée ; deadlock sans soupape ; renderedRef/DOM désync) + 8 corrections. Preuves : 1255 tests, e2e `scripts/e2e-legends-multistage.mjs` 34/34 (vraies routes, idempotence, gates), Playwright OK. **Démo isDev `demo-two-stage` EN BASE** (poules jouées + demie EN STREAM, rejouable `scripts/seed-demo-two-stage.mjs`) → à faire valider visuellement par Matt. PIÈGES : le CompetitionForm ne propose PAS la création multi-étapes (payload `stages[]` via API/scripts en attendant la refonte Opus) ; `replace_team` refusé après l'étape 1 ; `withdraw` d'une équipe déjà éliminée = statut seul, pas de cascade ; les libellés de l'adaptateur dé-préfixent les refs (`stage-ids`). Voir mémoire `project_tournament_view_scalable` § RUNTIME MULTI-ÉTAPES. — L'usine à tournois est le premier chantier de la plateforme générique de création de tournois (vision « Challonge/start.gg en mieux, à la sauce Aedral » — sources de vérité : [docs/plateforme-tournois-vision.md](docs/plateforme-tournois-vision.md) + [docs/registry-formats-design.md](docs/registry-formats-design.md), TOUJOURS les lire avant de toucher aux formats). Livré sur main (`1a7210c`+`77443e3`) : moteur PUR `lib/tournament/round-robin.ts` (poules serpentines + méthode du cercle + aller-retour, 4-64 équipes) + classement dédié `round-robin-standings.ts` (mini-championnat entre ex æquo, placements `rank{K}` fusion des plages inter-poules) + **registry de formats** `lib/competitions/formats.ts` (fiches déclaratives : configFields/presets/capabilities/summarize) et `formats-server.ts` (engines par kind) — les 2 formats existants migrés dedans, comportement inchangé. Types séquence de phases posés (décision A : tournoi = suite d'étapes ; runtime = chantier suivant). Review adversariale critique 52 agents → 13 défauts corrigés avec régressions (1167 tests verts). **PIÈGE CENTRAL : tout nouveau code lisant `isFinished`/`needsAdminDecision`/`computePlacements` DOIT router par `engineFor(kindOf(format))` (formats-server) — la console avait été oubliée, prouvé par la review.** Le formulaire admin ne propose PAS encore round_robin (gating naturel voulu). **Restent, dans l'ordre : vue des poules (viewer natif + table de classement Aedral — AVANT tout tournoi RR réel), moteur Suisse (Fable), runtime multi-étapes, seeding MMR, refonte page de création pilotée par la registry (Opus, UX non-technicien — future feature premium structures).** Voir mémoire `project_tournament_view_scalable`. **✅ REFONTE CONSOLE ADMIN COMPÉT — COMPLÈTE + EN PROD (21-22/07)** : les « petits retours UI/UX Matt » ci-dessus sont traités par un chantier dédié en 4 lots (bugs jour-de-match ; bracket interactif + panneau de détail ; inspection des équipes en dépliage sur place avec roster/MMR/staff ; disposition « salle de contrôle » bracket + rail collant via container query). Source de vérité : [docs/console-refonte.md](docs/console-refonte.md) + mémoire `project_console_refonte`. Reste optionnel : extraction du fichier console (~1750 l) + une démo rattachée à un circuit (pour voir les vrais points de barème). Piège : l'auth admin Playwright headless n'établit pas `isAdmin` → contrôle visuel de la console = en prod uniquement.
- **Springs Mania Cup — billetterie HelloAsso : EN PRODUCTION sur `main`, inscriptions ouvertes.** La branche `claude/hello-asso-springs-mania-fee6b4` **et son worktree ont été SUPPRIMÉS le 12/08** après vérification (`git log main..<branche>` vide — zéro commit hors de `main`). Il ne reste donc **qu'un seul endroit où travailler : `main`**. LAN Trackmania des 3-4 octobre 2026 à Marzy, 64 joueurs, 30 €, dotation 1200 €, organisée par SPRINGS E-SPORT. Sources de vérité : [docs/billetterie-mania-cup.md](docs/billetterie-mania-cup.md) (mode d'emploi de Matt, à jour) + [docs/reglement-mania-cup.md](docs/reglement-mania-cup.md) (règlement publié en v3 bilingue) + mémoire `project_mania_cup_helloasso`. **LE POINT D'ARCHITECTURE À NE JAMAIS CONTOURNER : HelloAsso ne signe pas les notifications envoyées aux comptes associatifs** (signature HMAC réservée à ses partenaires, vérifié sur `dev.helloasso.com/docs/secure-webhook` + SDK officiel) → le corps d'un webhook est fabricable par quiconque connaît l'adresse. Le webhook n'est qu'une sonnette : **tout ce qui décide d'une place est relu chez HelloAsso par un appel sortant authentifié** (`GET /v5/orders/{orderId}`). `lib/helloasso/reconcile.ts` DÉCIDE (pur, 36 tests), `apply.ts` ÉCRIT (transactionnel, idempotent par `itemId` — HelloAsso rejoue jusqu'au 200) ; le webhook et le bouton « Relire les commandes » empruntent le MÊME chemin d'écriture. Livré : connecteur OAuth avec jeton caché en base (plafond de jetons simultanés par clé), route `/api/mania-cup/webhook/[secret]`, réconciliation + rattachement manuel avec suggestions à une lettre près (jamais automatique : deux codes peuvent être voisins), console (recherche, filtres, export d'émargement, check-in, place, mot au joueur), badges imprimables (joueur/accompagnant/spectateur/staff), identité civile + contact d'urgence + droit à l'image collectés, `registrationCode` rendu unique par collection réservataire `mania_cup_codes` (l'identifiant EST le code), `lib/csv.ts` centralise l'anti-injection de formules (l'export d'audit n'en avait aucune). **Conservation corrigée** : autorisation parentale 1 an (elle partait à J+14), scan de la pièce du parent supprimé dès validation, références des titres dans `mania_cup_identity` (jamais servie avec « le dossier »). Rules déployées + vieux site resynchronisé. **PIÈGE : le PUT de `/api/mania-cup/settings` est PARTIEL** — un champ absent du corps garde sa valeur, sinon l'écran des tarifs efface la correspondance HelloAsso (même bug que `organizer`, cf. `project_affiche_resultat`). **DÉCISION DE MATT, prise en connaissance de cause : PAS de déclaration au ministère de l'Intérieur.** Ne pas y revenir, ne pas remettre ce type de démarche dans les livrables. **INSCRIPTIONS OUVERTES AU PUBLIC le 06/08** (`MANIA_CUP_PUBLIC=true` posé en production, déploiement `7095706`) : `/mania-cup`, `/mania-cup/inscription` et la carte « Sur place » de `/competitions` sont vérifiées accessibles en anonyme, et le raccourci `/mania-cup/billet` redirige bien vers la campagne `springs-mania-cup-1`. Deux pièges rencontrés à l'ouverture, à ne pas refaire : (1) le réglage `ticketingUrl` en base pointait encore sur la campagne dupliquée puis abandonnée (`…-1-joueur`) — un lien de billetterie doit être RELU en base avant toute publication, pas supposé ; (2) **toutes les variables d'environnement de ce projet Vercel sont « Sensitive »** → `vercel env pull` les rend VIDES, on ne peut jamais relire une valeur : la seule vérification possible est le comportement du site déployé, et en cas de doute on re-pose la variable plutôt que de supposer. **PREMIER PAIEMENT RÉEL ENCAISSÉ ET CONFIRMÉ le 06/08** — et il a révélé un bug de conception, corrigé (`1a7f316`) : la correspondance des tarifs étant vide, `decideItem` laissait `PaymentRecord.ticket` à null, le rattachement manuel testait `payment.ticket === 'player'`, aucune branche ne s'exécutait, l'inscription n'était jamais confirmée — **et la route répondait quand même `{ok:true}`**. Le joueur, ne voyant pas sa place, **a payé deux fois**. LEÇON GÉNÉRALE : **un outil de SECOURS ne doit jamais dépendre de ce qui vient d'échouer**. Désormais `resolveManualTicket` (pur, 6 tests) laisse l'organisation trancher la catégorie et refuse EXPLICITEMENT sinon ; `PaymentRecord.ticket` est typé `TicketKind | null` (le `string | null` d'avant est ce qui a laissé la comparaison passer en silence) ; un rejeu rafraîchit le journal sans renotifier. Correspondance des tarifs et URL de notification FAITES. **AUDIT DU CHEMIN DE PAIEMENT dans la foulée** (90 agents, 19 confirmés / 24 écartés) : **6 défauts d'argent corrigés et déployés** (`e00e9bc`) — (1) l'écran des tarifs envoyait l'objet ENTIER sur un PUT partiel et se repliait sur les valeurs par défaut quand la lecture échouait, donc **enregistrer un prix pouvait effacer la correspondance des tarifs** (chemin le plus probable vers la panne de ce soir) ; (2) l'idempotence se jugeait au journal et non à l'EFFET, donc « Relire les commandes » ne réparait jamais un dossier ayant dérivé ; (3) un même règlement pouvait confirmer DEUX inscriptions ; (4) le rattachement d'un accompagnant écrivait un champ mort → **aucun badge imprimé le jour J** ; (5) la page joueur affichait « Payer » alors que son règlement attendait — **la cause directe du double paiement**. Restants (visibilité, pas d'argent) en mémoire + `tasks/whtuk1s7e.output` : aucune alerte sur un règlement orphelin, console plafonnée à 300 paiements, relecture qui écrase un rattachement manuel, `maxPlayers` jamais lu. **SESSION DU 07/08** : (1) **lien Discord Springs** remplacé (salon général) — au passage, l'ancien n'était PAS mort : le serveur a un écran d'accueil obligatoire, et Discord place les arrivants en `pending` tant qu'ils ne l'ont pas franchi. `isGuildMember` répondait « oui » pour eux, donc le site validait une condition qu'on exige justement pour pouvoir les joindre (ils ne voient aucun salon et n'auraient reçu aucune consigne du jour J). Nouvelle fonction `guildMembershipState` — DISTINCTE, pour ne pas toucher au module compétitions qui utilise `isGuildMember` — plus le cas affiché au joueur. (2) **règlement PUBLIÉ en v2** (`docs/reglement-mania-cup.md` → collection `rulebooks`, scope `event_mania-cup`), avec correction d'une contradiction : il promettait le format et les cartes « publiés avant l'ouverture des inscriptions » alors que tout le concept de la LAN est qu'ils restent secrets jusqu'au jour J. (3) **`remark-gfm` n'était installé nulle part** → AUCUN tableau markdown ne se rendait sur le site, l'aperçu de la console d'organisation compris : le registre RGPD du règlement s'affichait en une longue ligne de barres verticales, dans sa section la plus juridique. Composant unique `components/ui/Markdown.tsx` (le tableau défile dans son propre cadre, la page ne part jamais en travers) + test de composant + sonde. (4) **les 13 défauts restants de l'audit paiement sont TOUS traités** : alerte à l'organisation sur règlement orphelin (notification in-app + DM Discord + compteur or sur l'onglet Paiements, posée sur le chemin d'écriture unique et seulement quand la décision CHANGE) ; compteurs de console portant sur toute la collection au lieu des 300 derniers, troncature annoncée ; `manualDecisionWins` — une décision humaine prime sur une incertitude automatique, jamais sur un remboursement (sans quoi chaque relecture effaçait le rattachement manuel ET réveillait l'organisation) ; faux vert de l'accompagnant sans place libre ; `ApplyOptions.maxPlayers` mort retiré plutôt que laissé mentir ; notification au joueur après rattachement manuel. (5) **e2e `scripts/e2e-mania-cup.mjs`** — 27 vérifications contre la production, 22 en local, **sonde à 10 échecs** en remettant le code du 6 août. **PIÈGE : lancer `npm run dev` depuis un worktree le prive de `.env.local` (qui n'existe que dans le dépôt principal) → toutes les routes en 500.** **SESSION DU 07/08 — l'événement s'ouvre à l'Europe.** (1) **Lien Discord Springs** remplacé, et surtout : le serveur a un écran d'accueil obligatoire, Discord place les arrivants en `pending` tant qu'ils ne l'ont pas franchi, et `isGuildMember` répondait « oui » pour eux — le site validait donc une condition qu'on exige justement pour pouvoir les joindre. Nouvelle fonction `guildMembershipState` (distincte, pour ne pas toucher au module compétitions) + cas affiché au joueur. (2) **Règlement publié en v2**, avec correction d'une contradiction : il promettait le format et les cartes « avant l'ouverture des inscriptions » alors que tout le concept est qu'ils restent secrets. (3) **`remark-gfm` n'était installé nulle part** → AUCUN tableau markdown ne se rendait sur le site, l'aperçu de la console compris : le registre RGPD du règlement s'affichait en une ligne de barres verticales. Composant unique `components/ui/Markdown.tsx` + test + sonde. (4) **Les 13 défauts restants de l'audit paiement traités** : alerte à l'organisation sur règlement orphelin (notification + DM + compteur or sur l'onglet), compteurs portant sur toute la collection, `manualDecisionWins` (une décision humaine prime sur une incertitude automatique, jamais sur un remboursement), faux vert de l'accompagnant sans place, `maxPlayers` mort retiré, notification au joueur après rattachement manuel. (5) **e2e `scripts/e2e-mania-cup.mjs`** — 27 vérifications contre la production, sonde à 10 échecs. (6) **Horaires du samedi corrigés** (9h et non 10h, étape de midi ajoutée) et **fuseau CEST écrit partout** — il ne l'était nulle part. (7) **Huit indices d'épreuves** publiés SANS VALIDATION puis retirés dans l'heure (voir mémoire `feedback_validation_contenu_public` — faute grave, le contenu éditorial ne se pousse jamais seul), refondus en cinq itérations, validés, republiés ; raisonnement dans `docs/epreuves-indices.md`. (8) **Badges au format 100 × 140 mm**, quatre par page — pas un simple agrandissement (largeur ×1,85, hauteur ×1,63) ; marge d'impression ramenée à 4 mm car 6 mm ne laissaient que 198 mm pour 200 mm de badges. Banc `scripts/mesure-badges.mjs` qui MESURE au lieu de regarder. (9) **Affiche refondue sans écusson** — le blason faisait penser à la ZRT ; titre centré souligné d'un trait, et les 281 px de vide mesurés sous le contenu rendus aux intervalles. Générateur mis à jour, donc régénérer ne peut plus ressortir l'ancien. (10) **Version anglaise** : affiche EN (`build_affiche.py en`), page `/mania-cup/en`, annonces Discord FR et EN. **i18n complète ÉCARTÉE** (66-85 h, ou 90-120 h avec next-intl) au profit du traducteur navigateur — décision de Matt, à raison. **Pays passés de 19 à 41** : ni Pologne, ni Suède, ni Finlande n'étaient acceptés, et le serveur validait contre cette même liste. **RESTE (08/08)** : traduire le règlement en anglais — **le français seul fait foi**, tranché par Matt — et corriger d'abord le bug de trace d'acceptation (`if (rulebook && !prev?.rulebookAccepted)` : un joueur qui re-coche après republication garde l'ancien numéro de version). Voir mémoire `project_mania_cup_international`. **RESTE côté Matt** : rembourser 30 € à Charly LEPRINCE (commande HelloAsso **188786961**, le doublon ; 188786965 vaut sa place) ; publier le règlement ; vérifier le quota 64 du tarif Joueur ; archiver l'ancienne campagne `springs-mania-cup-1-joueur`. **SESSION DU 07/08 (soir) — six chantiers, tous en production.** (1) **Console refondue** : lignes denses + dossier dépliable, et surtout QUATRE actions que le serveur savait déjà faire sans qu'aucun bouton ne les appelle (arrivée, emplacement, mot au joueur, retrait). Vrais drapeaux via `CountryFlag` — le composant existait, avec un commentaire décrivant exactement le défaut d'emoji sous Windows. (2) **Visuels ManiaPub** (`scripts/affiche-mania-cup/build_maniapub.py` + `render_maniapub.mjs`) : 3 formats × 2 langues, capture au double puis réduction, et un CONTRÔLE de rendu avant capture (débordement + alignement titre/dotation) qui a rattrapé trois défauts invisibles à l'œil, dont une grille CSS morte à cause d'un commentaire mal fermé. (3) **Compte Trackmania vérifié** — mémoire `project_trackmania_verification` : l'adresse trackmania.io se DÉDUIT de l'identifiant, la réclamer bloquait les inscrits de la LAN sur l'accueil ; badge sur profil + annuaire ; **bug silencieux corrigé** (la sync des trophées ne retrouvait les joueurs que par `tmIoUrl`, donc aucun inscrit LAN n'aurait jamais vu ses trophées). PIÈGE : `tmAccountId` ne prouve RIEN, seul `tmVerifiedAt` atteste. (4) **Liste d'attente** — mémoire `project_mania_cup_backlog` : l'invitation RÉSERVE la place (anti double-paiement), SANS horloge (décision Matt) → « Passer au suivant » est la SEULE libération. (5) **Alertes + rôle Discord** (`lib/mania-cup-alerts.ts`) : personne n'était prévenu d'une inscription — on n'alertait que sur les règlements qui ÉCHOUENT. (6) **Règlement v3 bilingue** publié + **bug de trace d'acceptation corrigé** (`lib/mania-cup-rulebook-trace.ts`). **SUITE DU 08/08** : (7) **numéro de commande HelloAsso affiché** dans le dossier, copiable — il était en base et déjà envoyé à la console depuis le premier paiement, mais affiché nulle part ; un joueur ayant payé deux fois a DEUX commandes et une seule tient sa place. (8) **Défaut de greffe corrigé** : le PATCH de `/api/admin/mania-cup` exige un `uid` et charge l'inscription AVANT tout, or j'avais placé les nouvelles actions après cette garde — « Rôles Discord » répondait « Requête invalide », et **les trois actions de liste d'attente auraient répondu « Inscription introuvable », l'invitation comprise** (invisible parce que la file est vide). Les actions qui ne visent pas une inscription se traitent désormais avant la garde ; les quatre éprouvées contre la production. (9) **Boutique de location reliée** : l'adresse était réglée et `TicketButton` acceptait `kind="rental"` depuis le début, mais **aucune page ne posait ce bouton** — le joueur lisait « la location se réserve sur HelloAsso » sans lien. Posé sur la carte « Tu amènes ton setup » (page publique, lien serveur) et dans l'espace du joueur à côté des accompagnants, avec rappel du code d'inscription. Le prix (90 €, réglé par Matt le 08/08) s'affiche sur les deux boutons, depuis le réglage et jamais d'une constante. **RÔLES DISCORD ACTIFS** : le rôle est configuré et les 4 inscrits réglés l'ont reçu (vérifié en production). **SESSION DU 08/08** : boutique de location BRANCHÉE de bout en bout (slug `HELLOASSO_SHOP_SLUG` — piège : la valeur avait été collée tronquée, et un formulaire introuvable était IGNORÉ EN SILENCE, une faute de frappe ressemblant à « pas de tarifs » ; la console l'annonce désormais). Les 3 articles (fixe 90 €/portable 60 €/écran 15 €) sont mappés sur `pc_rental`, et **le LIBELLÉ de l'article est conservé** — sans lui on savait qu'un joueur avait loué, pas quoi lui préparer. Numéro de commande HelloAsso affiché et copiable dans le dossier. **Défaut de greffe corrigé** : « Rôles Discord » et les 3 actions de liste d'attente étaient MORTES, placées après la garde qui exige un joueur cible. **RESPONSIVE de la LAN terminé et validé par Matt** : barre fixe (la coquille réserve 56 px pour le bouton de menu mais le contenu défilait dessous), menu déroulant au lieu d'une piste latérale, et échelle typographique redescendue SUR LA BASE UNIQUEMENT sur les 7 pages (paliers sm:/lg: intacts) — 5 mots par ligne → 7. Piège : un banc de mesure de débordement crie au loup sur un tableau qui défile dans son propre cadre et sur un halo en absolu ; vérifier `documentElement.scrollWidth`. **EN ATTENTE** : ajout au règlement sur la connexion Internet et la diffusion (Matt voit avec le président — un box 5G est peut-être prévu ; la règle « seule la diffusion Springs est autorisée » ne dépend PAS du box). **RESTE côté Matt** : renseigner le SALON d'alertes s'il ne l'est pas encore (le rôle, lui, est fait), rembourser Charly (commande **188786961**, surtout pas la 188786965 qui tient sa place), tester un paiement sur un tarif accompagnant/spectateur/location. **SESSION DU 12/08** : (1) **la location dit enfin CE QUI a été loué, et ne s'efface plus d'un clic** (`30070fc`) — le bouton « Poste loué » du dossier était un interrupteur déguisé en badge : un clic destiné à consulter le matériel a écrasé une location réglée 90 € par une location manuelle à 0 €. Le libellé (`LOCATION PC FIXE (2 jours)`) était en base depuis le premier paiement mais n'apparaissait que sur l'étiquette de la ligne repliée ; l'onglet Paiements le remplaçait même par le nom de sa catégorie. Désormais : article + montant + **commande de la LOCATION** (différente de celle de l'inscription), location payée non effaçable depuis la console (`isRentalPaid`, 409), rattachement manuel qui enregistre le libellé, saisie du matériel pour une location convenue de vive voix. Dossier réparé en production par « Relire les commandes » (12 lignes relues, 1 mise à jour) — **ce qui PROUVE en conditions réelles que la relecture répare un dossier qui a dérivé**, vérification qui restait en attente depuis le 6 août. (2) **Le chemin du REMBOURSEMENT** (`ebfac4c`, audit 35 agents contre les sources officielles) — déclencheur : « le paiement qui devait être remboursé a disparu de Aedral ». Il n'avait pas disparu : l'onglet Paiements s'ouvre sur « ce qui demande une décision », et un règlement remboursé n'en demande plus, donc **toute la caisse était masquée**. Mais l'audit a trouvé bien pire : **`cancelOrder` vaut FALSE par défaut** sur l'API de remboursement HelloAsso → un remboursement laisse la LIGNE à `Processed`, seul le PAIEMENT passe à `Refunded`, et nous ne lisions que les lignes → **un joueur remboursé gardait sa place sur 64**, son badge et son rôle Discord. Corrigé : lecture de `payments[]` (déjà renvoyé par les deux chemins) avec ventilation par ligne (rembourser une location ne fait plus sauter la place du même panier), **alerte et jamais révocation automatique sur un état de paiement** (un remboursement peut être partiel), état inconnu qui tient une place → décision humaine au lieu d'un « paiement en cours d'autorisation » faux, révocation qui n'est plus muette (joueur + organisation prévenus, rôle Discord retiré), remboursement d'un règlement rattaché à la main qui défait enfin quelque chose, inscription retirée qui ne ressuscite plus, changement de tarif qui ne déclasse plus la caisse encaissée, règlement annulé non rattachable, tri par dernier MOUVEMENT. Console : verdict à 3 états (fini le vert « rattaché » sur un remboursement), dates, états en français, porte de sortie « Voir toute la caisse ». 1672 tests + 7 tests de composant + e2e 35/35. Voir mémoire `project_mania_cup_helloasso`.

**RESTE côté code** : le webhook a fait ses preuves en production (deux rattachements automatiques) et le tarif **location** aussi (rattaché automatiquement le 12/08), mais les tarifs accompagnant / spectateur n'ont jamais vu un vrai paiement ; pas d'expiration des inscriptions impayées. **Prix et nombre de postes à louer en attente du sponsor Pages 404.**
